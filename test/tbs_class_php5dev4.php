...
static function f_Xml_AttMove(&$Txt, &$Loc, $AttDelim=false) {
  ...
  if ($AttDelim===false) $AttDelim = '"';
	...
	if ($Loc->AttForward) {
		$Loc->AttTagBeg += -$sz;
		$Loc->AttTagEnd += -$sz;
	} elseif ($Loc->PosBeg<$Loc->AttTagEnd) { // added for version 3.5.4-spe
		$Loc->AttTagEnd += -$sz;                // added for version 3.5.4-spe
	}
...
/*
		...
		if ($Loc->PosEnd<$Loc->AttBeg) $Loc->AttBeg += -$sz; // added for version 3.5.4-spe
		if ($Loc->PosEnd<$Loc->AttEnd) $Loc->AttEnd += -$sz; // added for version 3.5.4-spe
...
//			if ($Loc->AttForward) $Loc->AttValBeg += -$sz;
			if ($Loc->PosEnd<$Loc->AttValBeg) $Loc->AttValBeg += -$sz; // added for version 3.5.4-spe
