...
if (!isset($_GET['source'])) {
    exit("Enter the name of a TBS for PHP 5 file in argument 'source' in the URL. Usually: <a href=\"".$_SERVER['SCRIPT_NAME']."?source=tbs_class.php\">?source=tbs_class.php</a>");
}
...
function f_CheckVersionConsistency($SrcTxt, $Name, $v1, $v2)
{
    ...
    if ($v1_txt===false) {
        exit("Check $Name version consistency: $Name version not found ".$v1['info']);
    }
    $v2_txt= f_TextBetween($SrcTxt, $v2['prefix'], $v2['suffix']);
    if ($v2_txt===false) {
        exit("Check $Name version consistency: $Name version not found ".$v2['info']);
    }
    if ($v1_txt!==$v2_txt) {
        exit("Check $Name version consistency: '<b>".$v1_txt."</b>' is mentioned ".$v1['info']." while '<b>".$v2_txt."</b>' is mentioned ".$v2['info'].".");
    }
